...
class Geoformer(nn.Module):
    def __init__(self):
        super().__init__()
        hidden_dim = 1536
        self.backbone = timm.create_model('volo_d3_224', num_classes=0, drop_rate=0.1, drop_path_rate=0.1, features_only=True)
    ...
    def forward(self, x):
        ...
        pooled = [self.spatial_pool(fm).view(fm.size(0), -1) for fm in features]
        combined = torch.cat(pooled, dim=1)

        channel_weights = self.channel_attention(combined)
        attended = combined * channel_weights

        shared_features = self.shared_layers(attended)
        
        lat_lon = self.lat_lon(shared_features)
        
        outputs = {
            "latitude": lat_lon[:, 0],
            "longitude": lat_lon[:, 1],
            "quadtree_10_1000": self.quadtree(shared_features),
        }
        return outputs
